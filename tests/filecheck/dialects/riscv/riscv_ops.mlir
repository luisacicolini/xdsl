...
"builtin.module"() ({
  ...
  // CHECK-NEXT: ^bb0(%2 : !riscv.reg, %3 : !riscv.reg):
    ...
    %a, %b = riscv.pmov %0, %1 : (!riscv.reg, !riscv.reg) -> (!riscv.reg, !riscv.reg)
    // CHECK-NEXT: %{{.*}}, %{{.*}} = riscv.pmov %{{.*}}, %{{.*}} : (!riscv.reg, !riscv.reg) -> (!riscv.reg, !riscv.reg)
...
// CHECK-GENERIC-NEXT:      %a, %b = "riscv.pmov"(%0, %1) : (!riscv.reg, !riscv.reg) -> (!riscv.reg, !riscv.reg)
